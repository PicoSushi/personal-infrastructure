...
if hash toilet 2>/dev/null; then
    echo "${instance_name}" | toilet --font pagga --gay > /etc/motd
else
    echo "${instance_name}" > /etc/motd
fi

# Add user
adduser --shell /usr/bin/fish --disabled-password --gecos \"\" picosushi && sudo adduser picosushi sudo
echo 'picosushi ALL=(ALL) NOPASSWD:ALL' | sudo tee -a /etc/sudoers.d/picosushi-user
